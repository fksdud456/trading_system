...
class CreonTrade(SystemTrade):
    ...
    def buy_etf(self, code):
        ...
        try:
            ...
            target_price = self.get_target_price(code)  # 매수 목표가
            ma5_price = self.get_movingaverage(code, 5)  # 5일 이동평균가
            ma10_price = self.get_movingaverage(code, 10)  # 10일 이동평균가
            buy_qty = 0  # 매수할 수량 초기화
            if ask_price > 0:  # 매수호가가 존재하면
                buy_qty = self.buy_amount // ask_price
            stock_name, stock_qty = self.get_stock_balance(code)  # 종목명과 보유수량 조회
            ...
            if current_price > target_price and current_price > ma5_price \
                ...
                if ret == 4:
                    ...
                    return False
                time.sleep(2)
                printlog('현금주문 가능금액 :', self.buy_amount)
